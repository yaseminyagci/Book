...
namespace WebApp.Controllers.Api
{
    ...
    public class LikesController : Controller
    {
        ...
        public LikesController(IUnitOfWork unitOfWork, IRepository<Likes> repository, IMapper mapper, UserService userService,ILikesRepository likesRepository)
        {
        ...
        public async Task<ActionResponse> GetByBook(int id)
        {
            var userId = await _userService.GetAuthorizedUserIdAsync(User).ConfigureAwait(false);
            var data = await _repository.GetSingleFirstAsync(x => x.BookId == id && x.UserId == userId, x => x.UserDetail).ConfigureAwait(false);
            
            var getData = _mapper.Map<Likes, LikesGetData>(data);
            if (getData == null)
                getData = new LikesGetData() {Liked=false,Id=0 };
            int count= _repository.GetAll().Where(x => x.BookId == id&&x.Liked==true).Count();
        ...
        [HttpGet("GetAllbyTags")]
        public ActionResponse GetAllbyTags()
        {
             var data=_likesRepository.GetLikesByTag();
             var resultData = _repository.GetAll(x => x.Book, x => x.Book.Tags).GroupBy(x => x.Book.Title).Select(g=>new { name = g.Key, count = g.Count() }).ToList();
            //var resultData = _repository.GetAll(x => x.Book, x => x.Book.Tags).GroupBy(x => x.Book.Title).Select(g=>new { name = g.Key, count = g.Count() }).ToList();

            return new ActionResponse(data,StatusCodes.Status200OK);
