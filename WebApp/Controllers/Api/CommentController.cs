...
namespace WebApp.Controllers.Api
{
    ...
    public class CommentController : Controller
    {
        ...
        public async Task<ActionResponse> GetByBook(int id)
        {
            ...
            data = data.Where(c => c.BookId == id&&c.Status==(byte)RecordStatus.Active);
        ...
        {
            var data = _repository.GetAll().Where(x=>x.Status==1).Include(x=>x.UserDetail).Include(x=>x.Book);
        ...
        [HttpGet("GetAllTop10")]
        public async Task<ActionResponse> GetAllTop10()
        {
            var data = _repository.GetAll().Where(x => x.Status == 1).Include(x => x.UserDetail).Include(x => x.Book).Take(10);
            var result = _mapper.Map<List<Comment>, List<CommentGetData>>(await data.ToListAsync().ConfigureAwait(false));
            return new ActionResponse(result, StatusCodes.Status200OK);
